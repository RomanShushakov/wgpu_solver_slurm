...
wait_tcp4() {
  ...
  for _ in $(seq 1 "$tries"); do
    if nc -4 -vz "$host" "$port" >/dev/null 2>&1; then return 0; fi
    sleep "$sleep_s"
  done
  return 1
}
...
log "0) Preconditions"
[[ -f /etc/slurm/slurm.conf ]] || { echo "ERROR: /etc/slurm/slurm.conf not found. Run ./init_local.sh first."; exit 1; }
...
if ! wait_tcp4 "${SLURMDBD_ADDR}" "${SLURMDBD_PORT}" 30 1; then
  ...
  sudo journalctl -u slurmdbd -n 160 --no-pager || true
  sudo ss -lntp | egrep ":${SLURMDBD_PORT}\b|slurmdbd" || true
  exit 1
fi

log "2) Set AccountingStorageTRES (GPU allocation accounting) in slurm.conf"
...
# Ensure dbd is still reachable right before controller restart
wait_tcp4 "${SLURMDBD_ADDR}" "${SLURMDBD_PORT}" 10 1 || { echo "ERROR: slurmdbd became unreachable"; exit 1; }
